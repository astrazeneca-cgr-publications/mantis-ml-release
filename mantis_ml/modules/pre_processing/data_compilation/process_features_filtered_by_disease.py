...
class ProcessFeaturesFilteredByDisease(ProcessGenericFeatures):
    ...
    def process_gtex_features(self, save_to_file=False):
        ...
        pattern = self.cfg.tissue  # no spaces allowed

        all_tissues = self.cfg.additional_tissues + [self.cfg.tissue]
    ...
    def process_protein_atlas_features(self, include_terms, exclude_terms, verbose=False, save_to_file=False):
        ...
        tissue_str = self.cfg.tissue.lower()
    ...
    def run_all(self):
        ...
        if not self.cfg.generic_classifier:
            ...
            tissue_gwas_df = self.process_gwas_features(pattern_lists=pattern_lists, search_term=self.cfg.tissue)
            print('GWAS:', tissue_gwas_df.shape)


        # Protein Atlas
        protatlas_include_terms = [self.cfg.tissue]
        protatlas_include_terms.extend(self.cfg.seed_include_terms)
        ...
        # MSigDB
        msigdb_include_terms = seed_include_terms.copy()
        msigdb_include_terms.extend(self.cfg.additional_include_terms)
        msigdb_include_terms.extend([self.cfg.tissue])
